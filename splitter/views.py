...
def send_remainder(request, username, amount):
    ...
    if (res):
        messages.success(request,"Reminder had been sent!!!")
    else:
        messages.error(request,"Error!! Reminder had not been sent!!!")
    return HttpResponseRedirect(reverse('splitter:my_debts'))



def send_email(email, amount, curr_user):
    user = "nasvwadhwa@gmail.com"
    password = "hhtjzsudgmqnmyoq"
...
# def send_email(email, amount, curr_user):
#     user = "faresplit234@gmail.com"
#     password = "iwherhuzvdyunivz"
...
def paym(request):
    return render(request,'splitter/payment.html')
...
def paynow(request):
    ...
    if request.method == "POST":
        ...
        rec_amount = request.POST["rec_amount"]
        # print("Priting the info: ",rec_name, rec_amount)

        data = {'rec_name':rec_name, 'rec_amount':rec_amount}

    return render(request,"splitter/payment.html", context=data)
